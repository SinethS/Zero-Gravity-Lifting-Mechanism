...
void motor::trapspeedcontrol(float speed, float dt) {
    // Motion profile parameters (tune based on motor and crane)
    const float MAX_ACCEL = 500.0f; // Max acceleration (RPM/s)
    const float MAX_DECEL = 500.0f; // Max deceleration (RPM/s)
    const int MAX_RPM = 1000;       // Max motor RPM
    const int MIN_RPM = -1000;      // Min motor RPM
    ...
    // Set direction based on target RPM
    if (target_rpm > 0) {
        setDirection(true); // Forward
    } else if (target_rpm < 0) {
        setDirection(false); // Backward
        target_rpm = -target_rpm; // Make RPM positive for speed setting
    } else {
        // Target speed is zero, decelerate to stop
        target_rpm = 0;
    }

    // Calculate maximum speed change for this time step
    float max_change = MAX_ACCEL * dt; // Max speed change (RPM) based on acceleration
    ...
    // Convert to integer RPM for motor
    ...
    if (rpm > 0) {
        setSpeed(rpm);
        if (!running) {
            runMotor(); // Start motor if not running
            running = true;
        }
    } else {
        setSpeed(0);
    ...
    // Update current_rpm for next iteration
    current_rpm = rpm;
}